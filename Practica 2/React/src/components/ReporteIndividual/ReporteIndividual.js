	...
	render() {
        ...
        function TablaListado (){
            ...
            const handleInputChangeC = (event) => {
              carnetG.curso = event.target.value;
            }
                    ...
                    console.log(data)
                      ...
                      data.map(Item => {
                          if(Item.carne == carnetG.carne){
                            console.log(carnetG.curso.toLowerCase())
                            console.log(String(Item.curso).toLowerCase())
                            if(carnetG.curso.toLowerCase() == String(Item.curso).toLowerCase()){
                              listadoG.push({ carne: Item.carne, nombre: Item.nombre, curso: Item.curso, fecha: Item.fecha, cuerpo: Item.cuerpo, procesado: Item.procesado  });
                            }
                          }
                        }
                      )
                    
                    setMsg(listadoG.map(Item => ({ carne: Item.carne, nombre: Item.nombre, curso: Item.curso, fecha: Item.fecha, cuerpo: Item.cuerpo, procesado: Item.procesado  })))
            ...
            return(
              <div>
                <div style={divStyle}>
                  ...
                  <br/>
                  <br/>
                      <button style={button} >Reporte Individual</button>
                  </div>
                  <br/>
                  <br/>
                  <br/>
                  ...
                  <label style={text1}> Curso/Proyecto asignado:  
                    <input style={text1} placeholder="Curso/proyecto" className="form-control" name="curso" onChange={handleInputChangeC}></input>
                    <br/>
                    <br/>
                  </label>
                      ...
                      <TableHead>
                          ...
                          <TableBody>
                              {msg.map((row) => (
                                  ...
                                  <StyledTableCell component="th" scope="row">{row.cuerpo}</StyledTableCell>
